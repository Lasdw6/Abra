...
export async function run(argv,io=console){
  ...
  if(command==='export'){
    ...
    const state=from==='cdp'?await capture(await resolveCdpEndpoint(positionals[1]||need(flags,'cdp')),policy,{browserContextId:flags['browser-context-id']}):(from==='local'||from==='managed')?await captureFrom({type:from,profile:flags.profile,max_tabs:flags['max-tabs']},policy):(()=>{throw new Error('--from must be local, managed, or cdp');})();
